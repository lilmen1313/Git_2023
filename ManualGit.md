...
```
7. Для обновления локального репозитория существует команда (вытягивает изменения из удалённого репозитория)
```
git pull
```
...
## Pull Request
1. Переходим в удалённый репозиторий, в который планируем вносить изменения (командная работа над проектом). 

При этом нужно быть под своей учётной записью (залогининым на GitHub) 

2. В правом верхнем углу нажимаем кнопку **Fork**
![logo](Fork.png)
Создаем копию чужого репозитория в своём репозитории.
3. Переходим в эту копию (но уже в своем аккаунте) и копируем URL на этот репозиторий.
4. Переходим в терминал и выполняем команду
```
git clone <URL>
```
**ВНИМАНИЕ!** Выполнять команду в новой папке. Не клонировать в место, которое уже отслеживается Git.

5. Создать новую ветку (new branch), внести предложения в проект.
6. Зафиксировать изменения с комментарием.
7. Отправить изменения в удалённый репозиторий

*Внимание!* Команда для этого будет `git push -u origin <new branch>`, т.к. в удалённом репозитории нет такой ветки. Необходимо выполнить связь.

8. Переходим в удалённый репозиторий.
9. Выбираем вкладку Pull Request. Попадаем в Comparing changes.
10. Настраиваем откуда и куда будет направлен Pull Request.
Обратить внимаение на ветки. Если изменения не отражаются, то скорее всего не выбрана new branch.
11. При успешном выполнении всех этапов появится надпись Able to merge. Нажимаем **Create Pull Request**.